      ...
      pkgs.runCommand "nixos-boot-disk"
        { preVM =
            ''
              ...
              ${qemu}/bin/qemu-img create -f qcow2 $diskImage "${bootPartitionSize}M"
        ...
        ''
          # Create a /boot EFI partition with the size of 'bootPartitionSize'M and arbitrary but fixed GUIDs for reproducibility
...

{
  ...
  options = {
    ...
    # da sollte vlt ein qemu dazwischen
    # die option sollte womöglich irgendwo anders stehen
    virtualisation.bootPartitionSize = 
      mkOption {
        ...
        description =
          lib.mdDoc ''
            The memory size in megabytes of the boot partition.
          '';
      };
